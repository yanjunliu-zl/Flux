...
Usage:
    ...
    python -m seedance.tools.download_webvid --metadata_only \
        --output data/webvid/

    # Download videos with captions (slow, needs network)
    python -m seedance.tools.download_webvid --max_videos 5000 \
        --output data/webvid/ --workers 8

    # Stream metadata only (no download) — for checking stats
    python -m seedance.tools.download_webvid --stream
"""
...
def main():
    ...
    print(f"  python -m seedance.tools.quality_filter --input {video_dir} --output data/filtered/")
