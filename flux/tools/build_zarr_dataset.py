...
Usage:
    ...
    python -m flux.tools.build_zarr_dataset \
...
def build_zarr_dataset(
    ...
    if extract_lfa:
        try:
            from flux.models.lfa_encoder import LFAEncoder
            device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    if extract_kp:
        try:
            from flux.models.kp_encoder import KP3DEncoder, KPConfig
            device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    if encode_vae:
        try:
            from flux.models import VideoVAE, AudioVAE
            device = "cuda" if torch.cuda.is_available() else "cpu"
